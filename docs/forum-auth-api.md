...
* **модерация форума** — узел права `forum.moderate`. Звёздочки работают как в игре:
  игроку с `*` или `forum.*` выдавать ничего не нужно.

Выдать модератора форума роли:

```sql
INSERT INTO perms.role_permission (role_id, permission)
SELECT id, 'forum.moderate' FROM perms.role WHERE key = 'moderator';
```
